...
function toggleButtonVisibility(keywordCount) {
    ...
    toggleKeywordBtn.textContent = keywordCount > 0 ? `Hide blocked (${keywordCount})` : "Show blocked";
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('saveBtn').addEventListener('click', () => addKeyword(getKeyword()));

    document.getElementById('toggleKeywordBtn').addEventListener('click', function () {
        const keywordList = document.getElementById('keywordList');
        const isVisible = keywordList.style.display !== 'none';
        keywordList.style.display = isVisible ? 'none' : 'block';
        this.textContent = isVisible ? `Show blocked (${data.keywords.length})` : `Hide blocked (${data.keywords.length})`;
    });
